...
const VerifyAccount = ({ email, URL }: VerifyAccountProps) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-400 via-white to-green-500 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 py-4 sm:py-8 px-3 sm:px-4 transition-colors duration-300">
      <div className="bg-white/90 dark:bg-gray-800/95 shadow-2xl rounded-2xl sm:rounded-3xl px-4 sm:px-6 md:px-10 py-6 sm:py-8 md:py-10 w-full max-w-md mx-auto backdrop-blur-sm border border-white/20 dark:border-gray-700/50">
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex flex-col items-center mb-6 sm:mb-8">
            ...
            <div className="mt-2 sm:mt-3 h-1 w-12 sm:w-16 bg-gradient-to-r from-blue-400 to-blue-600 dark:from-blue-500 dark:to-blue-400 rounded-full"></div>
          </div>

          {/* Verification Code Section */}
          <div className="flex flex-col items-center gap-4 sm:gap-6">
            <div className="text-center w-full">
              <label className="text-sm sm:text-base font-semibold text-gray-700 dark:text-gray-300 mb-3 sm:mb-4 block">
                {t("verificarcuentacodigo")}
              </label>
              <div className="inline-flex items-center gap-1.5 sm:gap-2 bg-green-50 dark:bg-green-900/20 px-2 sm:px-3 py-1 rounded-full border border-green-200 dark:border-green-700 max-w-full">
                <svg
                  className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-green-600 dark:text-green-400 flex-shrink-0"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                ...
                <span className="text-green-700 dark:text-green-300 font-semibold text-xs sm:text-sm truncate">
                  {email}
                </span>
              </div>
            </div>
            ...
            {/* Code Input Fields */}
            <div className="flex gap-1.5 sm:gap-2 md:gap-3 justify-center w-full px-2">
              {code.map((digit, idx) => (
                <input
                  key={idx}
                  ref={(el) => {
                  ...
                  inputMode="numeric"
                  maxLength={1}
                  className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 text-center text-xl sm:text-2xl font-bold border-2 border-green-300 dark:border-gray-600 rounded-lg sm:rounded-xl focus:ring-2 focus:ring-blue-400 dark:focus:ring-blue-500 focus:border-blue-400 dark:focus:border-blue-500 outline-none bg-green-50 dark:bg-gray-700 text-gray-900 dark:text-white transition-all duration-200 hover:border-green-400 dark:hover:border-gray-500"
                  value={digit}
            ...
            {error && (
              <div className="w-full p-2.5 sm:p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700 rounded-lg sm:rounded-xl">
            ...
            {success && (
              <div className="w-full p-2.5 sm:p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded-lg sm:rounded-xl">
                <div className="flex items-center gap-2 justify-center">
                  <svg
            ...
            <button
              type="submit"
              className="w-full bg-gradient-to-r from-green-500 to-green-600 dark:from-green-600 dark:to-green-700 hover:from-green-600 hover:to-green-700 dark:hover:from-green-700 dark:hover:to-green-800 text-white font-bold py-3 sm:py-4 rounded-lg sm:rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2 sm:gap-3 group text-sm sm:text-base"
            >
              <svg
